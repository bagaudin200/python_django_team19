...
class Order(models.Model):
    ...
    PAYMENT_TYPES = (
        ('card', 'card'),
        ('random', "random someone's account"),
    )

    STATUS_CREATED = 'created'
    STATUS_OK = 'ok'
    STATUS_INVALID_CARD_NUMBER = 'invalid_card_number'
    STATUS_CHOICES = [
        (STATUS_CREATED, 'created'),
        (STATUS_OK, 'ok'),
        (STATUS_INVALID_CARD_NUMBER, 'invalid card number'),
    ]
    ...
    status = models.CharField(max_length=50, choices=STATUS_CHOICES, default=STATUS_CREATED, verbose_name='status')
